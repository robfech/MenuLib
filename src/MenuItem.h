...
public:
  MenuItem(MenuItem *parent, const FlashString *text)
      : parent(parent), enabled(true), edited(false) {
    setText(text);
  }
  ...
  virtual void setText(const FlashString *text) {
    this->text = reinterpret_cast<PGM_P>(text);
  }

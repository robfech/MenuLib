...
button_event_t buttonEvent() {
  ...
  case 'd':
    return BTN_INCR;
  case 'q':
    return BTN_DECR_FAST;
  case 'e':
    return BTN_INCR_FAST;
  case ' ':
    return BTN_SEL;
  case 's':
    return BTN_ESC;
...
void setup() {
  ...
  sub->addItem(new NumericSelectorUint8(sub, F("uInt8"), test_uint8, 0, 255, 1,
                                        10, F("ms"), numberSelectedCallback));
  sub->addItem(new NumericSelectorInt16(sub, F("int16"), test_int16, 0, 10000,
                                        10, 1000, 0, F("mm"),
                                        numberSelectedCallback));
  sub->addItem(new NumericSelectorInt16(sub, F("intDec"), test_deci, -100, 100,
                                        1, 20, 2, F("kg"),
                                        numberSelectedCallback));
  sub->addItem(new NumericSelectorList(sub, F("list"), test_arr_val,
                                       test_arr_max, test_arr));
...
void test_action() {
  ...
  for (int i = 0; i < 10; i++) {
    // if select or return button is pushed exit the function and go back to
    // menu
    button_event_t event = buttonEvent();
    if ((event == BTN_ESC) || (event == BTN_SEL))
      break;
